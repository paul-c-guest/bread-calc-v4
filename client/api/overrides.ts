...
export const getOverridesForOwner = async (token: string) => {
...
export const createOverride = async (override: Override) => {
  const response = await request.post("/api/v1/overrides").send(override)
  return response.body
}

export const updateOverride = async (override: Override) => {
